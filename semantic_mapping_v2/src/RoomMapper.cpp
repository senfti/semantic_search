...
void RoomMapper::updateOctoMaps(){
...
void RoomMapper::enableObjectMapping(bool enable){
  if(enable)
    vision_sub_ = node_.subscribe("vision_result", 1, &RoomMapper::visionCb, this);
  else
    vision_sub_.shutdown();
}


void RoomMapper::enableOctoMapping(bool enable){
  if(enable)
    cloud_sub_ = node_.subscribe("/camera/depth_registered/points", 1, &RoomMapper::cloudCb, this);
  else
    cloud_sub_.shutdown();
}


void RoomMapper::startSlam(bool enable){
  if(enable)
    startLiveSlam();
...
void RoomMapper::enable(){
  startSlam(true);
  enableOctoMapping(true);
}


void RoomMapper::disable(){
