...
//      base_obj_prob_pub_.resize(ObjectMapper::getObjNames().size());
//      for(int i=0; i<ObjectMapper::getObjNames().size(); i++){
...
void HierarchyMapper::publishObjProbMap(const ObjectMap &map, int idx){
  if(obj_prob_pub_.empty()){
    obj_prob_pub_.resize(ObjectMapper::getObjNames().size());
    for(int i=0; i<ObjectMapper::getObjNames().size(); i++){
                                                                 ...
                                                                 const cv::Point& new_orig, const cv::Size& new_size, int num_obj_types, int obj)
{
  ...
  if(obj >= 0){
    int o = obj;
    obj_from_room_maps.push_back(cv::Mat_<float>(complete_room_type_map[0].rows, complete_room_type_map[0].cols, 0.f));
    cv::Mat_<float> inv = cv::Mat_<float>(complete_room_type_map[0].rows, complete_room_type_map[0].cols, 0.f);
    ...
    cv::GaussianBlur(obj_from_room_maps.back(), inv, cv::Size(k_size,k_size), CELL_TO_OBJ_PROB_GAUSSIAN_SIGMA, CELL_TO_OBJ_PROB_GAUSSIAN_SIGMA, cv::BORDER_REPLICATE);
    inv(cv::Rect(new_orig.x, new_orig.y, new_size.width, new_size.height)).copyTo(obj_from_room_maps.back());
  }
  else{
...
bool HierarchyMapper::objMapSrvCb(semantic_mapping_v2::ObjectMapSrv::Request& req, semantic_mapping_v2::ObjectMapSrv::Response& res){
  ...
  std::vector<cv::Mat_<float>> obj_prob_2d_area;
  if(OBJ_TO_ROOM)
    obj_prob_2d_area= get2dAreaObjProbMaps(obj_maps, behind_door_mask, occ_map, room_type_maps[0].getOrigin(),
                                                                       room_type_maps[0].getWidth(), room_type_maps[0].getHeight(), ROOM_CELL_OBJ_KERNEL_SIZE);

  std::vector<cv::Mat_<float>> obj_based_room_type_map;
  if(OBJ_TO_ROOM)
    obj_based_room_type_map = getObjBasedRoomTypeMap(obj_prob_2d_area, room_type_maps.size());
...
void HierarchyMapper::publishDebug(const cv::Mat_<float>& behind_door_mask, const ObjectMap& occ_map, const std::vector<cv::Mat_<float>>& obj_prob_2d_area,
  ...
  if(!minimal){
    ...
    if(base_obj_prob_pub_.empty()){
      base_obj_prob_pub_.resize(ObjectMapper::getObjNames().size());
      for(int j = 0; j < ObjectMapper::getObjNames().size(); j++){
...
//      cv::bitwise_and(255-occupancy_map_obj*64, occ_dil(cv::Rect(3*dil_size,3*dil_size,occupancy_map_obj.cols,occupancy_map_obj.rows))*255, occupancy_map_obj);
  ...
  if(obj_maps[i][0].getOrigin() != room_type_maps[i][0].getOrigin())
    cv::copyMakeBorder(occupancy_map_room, occupancy_map_room, std::max(room_type_maps[i][0].getOrigin().y*4 - obj_maps[i][0].getOrigin().y*4, 0), 0,
                       std::max(room_type_maps[i][0].getOrigin().x*4 - obj_maps[i][0].getOrigin().x*4,0), 0, cv::BORDER_CONSTANT, cv::Scalar(0));
  if(occupancy_map_obj.cols != room_type_maps[i][0].getWidth()*4 || occupancy_map_obj.rows != room_type_maps[i][0].getHeight()*4)
    cv::copyMakeBorder(occupancy_map_room, occupancy_map_room, 0, std::max(room_type_maps[i][0].getHeight()*4 - occupancy_map_room.rows,0), 0, std::max(room_type_maps[i][0].getWidth()*4 - occupancy_map_room.cols,0), cv::BORDER_CONSTANT, cv::Scalar(0));
...
//  }
  ...
  msg.room_type_probs = std::vector<float>(NUM_ROOM_TYPES, 1.f/NUM_ROOM_TYPES);
...
bool HierarchyMapper::hierarchySrvCb(semantic_mapping_v2::HierarchySrv::Request& req, semantic_mapping_v2::HierarchySrv::Response& res){
  ...
  std::vector<std::vector<float>> base_obj_probs(ObjectMapper::getObjNames().size());
  ...
  for(int i=0; i<grid_maps.size(); i++){
    ...
    std::vector<cv::Mat_<float>> obj_prob_2d_area;
    if(OBJ_TO_ROOM)
      obj_prob_2d_area = get2dAreaObjProbMaps(obj_maps[i], behind_door_mask, occ_map, room_type_maps[i][0].getOrigin(),
                                              room_type_maps[i][0].getWidth(), room_type_maps[i][0].getHeight(), ROOM_CELL_OBJ_KERNEL_SIZE);
    std::vector<cv::Mat_<float>> obj_based_room_type_map;
    if(OBJ_TO_ROOM)
      obj_based_room_type_map = getObjBasedRoomTypeMap(obj_prob_2d_area, room_type_maps[i].size());
    ...
    if(DEBUG_OUTPUT){
      ...
      if(PUBLISH_DEBUG_IMAGES && (req.debug_room == i || (req.debug_room==-1 && current_mapper_==i))){
        publishDebug(behind_door_mask, occ_map, obj_prob_2d_area, obj_based_room_type_map, complete_room_type_map, room_based_obj_map, room_type_cell_number,
                     complete_obj_map, dummy_occ_map, grid_maps, obj_maps, room_type_maps, octomaps[i], only_laser_points, i, req.debug_room==-1);
      }
    }
...
void HierarchyMapper::hierarchyTestCb(const semantic_mapping_v2::HierarchyTestMsgConstPtr &msg){
  ...
  for(int i=0; i<grid_maps.size(); i++){
    ...
    if(PUBLISH_DEBUG_IMAGES && msg->output_room == i){
      publishDebug(behind_door_mask, occ_map, obj_prob_2d_area, obj_based_room_type_map, complete_room_type_map, room_based_obj_map, room_type_cell_number,
                   complete_obj_map, dummy_occ_map, grid_maps, obj_maps, room_type_maps, octomaps[i], only_laser_points, i, false);
    }
