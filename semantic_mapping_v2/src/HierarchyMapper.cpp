...
std::vector<float> HierarchyMapper::getCompleteObjProbs(const std::vector<ObjectMap>& complete_obj_map, std::vector<float> room_type_probs, const ObjectMap& occ_map, int unseen_estimate){
  ...
  for(int o=0; o<complete_obj_map.size(); o++){
    ...
    for(int r = 0; r < room_type_probs.size(); r++){
      unseen_prob_estimate += room_type_probs[r] * RoomMapper::getObjProbGivenRoomPerCell(o,r);
    }
    complete_obj_probs[o] = std::min(complete_obj_map[o].getObjectProb(occ_map, unseen_prob_estimate, unseen_estimate, false), 0.999f);
  }
...
bool HierarchyMapper::objMapSrvCb(semantic_mapping_v2::ObjectMapSrv::Request& req, semantic_mapping_v2::ObjectMapSrv::Response& res){
  ...
  cv::Mat_<float> behind_door_mask = getBehindDoorMask(doors[req.room_id], obj_maps[req.room_id][0]);
  ObjectMap occ_map(obj_maps[req.room_id][0].getResolution(), obj_maps[req.room_id][0].getBaseSize(), obj_maps[req.room_id][0].getWidth(), obj_maps[req.room_id][0].getHeight(),
                    obj_maps[req.room_id][0].getOrigin(), obj_maps[req.room_id][0].getMaxHeight(), octomaps[req.room_id], doors[req.room_id]);
...
int HierarchyMapper::estimateUnseen2dCells(const nav_msgs::OccupancyGrid& map, const ObjectMap& obj_map, const cv::Mat_<float> behind_door_mask, bool explored, int& search_cells){
  ...
  if(explored){
    ROS_ERROR("UNSEEN %d", only_laser);
    search_cells = seen + only_laser;
    return only_laser;
  }
  else{
    ...
    ROS_ERROR("UNSEEN val %d est %d min_unseen %d only_laser %d seen %d", std::max(estimate - seen, only_laser+min_unseen), estimate, min_unseen, only_laser, seen);
    search_cells = std::max(estimate, only_laser+min_unseen+seen);
    return std::max(estimate - seen, only_laser+min_unseen);
  }
}
...
bool HierarchyMapper::hierarchySrvCb(semantic_mapping_v2::HierarchySrv::Request& req, semantic_mapping_v2::HierarchySrv::Response& res){
  ...
  for(int i=0; i<grid_maps.size(); i++){
    ...
    std::vector<float> complete_obj_probs = getCompleteObjProbs(complete_obj_map, complete_room_type_probs, occ_map, num_unseen);
    std::vector<float> complete_obj_probs2 = getCompleteObjProbs(complete_obj_map, complete_room_type_probs, occ_map, 0);
    ...
    if(PUBLISH_DEBUG_IMAGES && req.debug_room == i){
      publishDebug(behind_door_mask, occ_map, obj_prob_2d_area, obj_based_room_type_map, complete_room_type_map, room_based_obj_map, complete_room_type_probs,
