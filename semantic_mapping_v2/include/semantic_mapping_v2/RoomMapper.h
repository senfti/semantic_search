...
class RoomMapper : public SlamGMapping{
  public:
    ...
    static double getObjProbGivenRoomPerCell(int obj, int room, bool recalc = false, float recalc_fill = -1.f);
    static std::vector<std::vector<double>> prob_map_per_2d_cell;
    static double getObjProbGivenRoomObjPriorPerCell(int obj, bool recalc = false);
    static double getObjProbGivenRoomRoomPriorPerCell(int room, bool recalc = false);
  ...
  public:
    ...
    int getBestParticleIdx() {
      boost::lock_guard<boost::mutex> lock(gsp_mutex_);
      return std::min(gsp_->getBestParticleIndex(), int(octo_maps_.size())-1);
    }
    GMapping::OrientedPoint getParticlePose2D(int particle_idx, ros::Time time);
    tf::Transform getParticlePose3D(int particle_idx, ros::Time time);
    GMapping::OrientedPoint getBestParticlePose2D(ros::Time time);
    tf::Transform getBestParticlePose3D(ros::Time time);
    ...
    visualization_msgs::MarkerArray getObjectProbMsg(int id);
    visualization_msgs::MarkerArray getRoomProbMsg(int id);
    geometry_msgs::PoseArray getParticlePoseMsg();
    ...
    OctoMapper getOctomap(){
      boost::lock_guard<boost::mutex> maps_lock(maps_mutex_);
      return octo_maps_[getBestParticleIdx()]->getCopy();
    }
    std::vector<Door> getDoors(){
      boost::lock_guard<boost::mutex> maps_lock(maps_mutex_);
      return door_mappers_[getBestParticleIdx()]->getDoors();
    }
    std::vector<ObjectMap> getObjMaps(){
      boost::lock_guard<boost::mutex> maps_lock(maps_mutex_);
      return obj_mappers_[getBestParticleIdx()]->getMaps();
    }
