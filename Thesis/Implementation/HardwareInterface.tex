...
The ASUS XTIon Pro-rgbd-cameras comunicate with the rest of the system via the openni2-package. The nodes of this package opens the camera and transforms the raw data from the camera into various ROS-messages. The RGB-image and the depth-registered point cloud of the main camera are used by the vision node. The depth-registration assigns a 3D point to every pixel in the RGB-image. For pixels where this is not possible, the point is set to (NaN,NaN,NaN). The point cloud from the upward looking camera is sent to the door{\_}detection node. To reduce the computational cost, the frame rate of the camera was reduced to 5 frames per second, as more images cannot be processed anyway.
