...
The time necessary for exploration is ignored, as the exploration is difficult to model. On the one hand it takes time, but on the other hand some objects might be found during the process and information about other rooms and likely object locations is gathered. Ignoring the time for exploration seems to be a good idea. Another used idea concerning exploration is setting the high-level attributes of unvisited rooms to very optimistic values. This is done to encourage the robot to peek into rooms. Peeking into a room is a desirable behavior as it takes relatively little time and gives the robot much more information about the environment. 
...
The search ends if the object is found or all areas in the room are covered. Therefore the probability distribution of ending the search over time $f_r(t)$ is
\begin{equation}
